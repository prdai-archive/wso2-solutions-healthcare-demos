# care-loop-questionnaire-agent

Standalone Ballerina agent (port 8000). `POST /questionnaires {patientId}`
...
`hl7-ai-competition/care-loop-questionnaire-agent`, and port 8000 (AMP's
default agent port; `listenPort` defaults to it). AMP injects configuration as
environment variables; Ballerina maps `BAL_CONFIG_VAR_<NAME>` to the matching
