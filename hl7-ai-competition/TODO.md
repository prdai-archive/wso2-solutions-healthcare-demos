...
- Verify the agent packages end-to-end against a live Agent Manager: gateway Bearer-vs-API-Key behavior on real LLM calls, MCP proxy auth from McpToolKit, and trace export into the AMP observability stack.
...
- care-loop-risk-agent's /risk-assessment (GPT_4_1) still occasionally cites a resource that doesn't quite support its own claim - worth trying a bigger/different model or an audit pass to close this out fully.
