...
Copy `Config.toml.example` to `Config.toml` (gitignored), set `modelProvider`, and fill in the matching API key. The provider is chosen in `Config.toml`, not by any environment variable: `openai` calls OpenAI directly by default (routed through the AMP gateway only if `openAiServiceUrl` points at it), `anthropic` calls the Anthropic API directly, and `anthropic-amp` uses the same stock provider pointed at the AMP gateway (`anthropicServiceUrl = http://amp:22893/careloop-anthropic/v1`), whose `careloop-anthropic` gateway provider is registered to accept the stock provider's native `x-api-key` header. docker-compose mounts `Config.toml` into the container, so this file must exist before `make up`/`docker compose up` will start this service. See the WSO2 Agent Manager section of the main README for the AMP gateway, which is opt-in behind the `amp` compose profile.
