...
// analysis-service encodes the real per-request threshold/outcome into
// prediction[0].rationale (see buildMlRiskAssessment). Records saved before
// that field existed won't match, so fall back to "—" rather than guessing.
...
// "risk=<band>" (see fhir_resources.bal). Anything else means no real band recorded.
